...
export class PanoramaEquirectangularComponent implements AfterViewInit {
  ...
  @Input()
  public widthSegments: number = 120;

  @Input()
  public heightSegments: number = 80;

  @Input()
  public radius: number = 480;

  @Input()
  public texture: string = '/assets/textures/tears_of_steel_bridge.jpg';
  ...
  private rotateCamera() {
    ...
    this.latitude = Math.max(-58, Math.min(58, this.latitude));
    this.phi = THREE.Math.degToRad(90 - this.latitude);
    this.theta = THREE.Math.degToRad(this.longitude);

    this.cameraTarget.x = 480 * Math.sin(this.phi) * Math.cos(this.theta);
    this.cameraTarget.y = 480* Math.cos(this.phi);
    this.cameraTarget.z = 480 * Math.sin(this.phi) * Math.sin(this.theta);
